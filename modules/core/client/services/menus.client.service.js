  ...
  function Menus() {
    ...
    function addMenuItem(menuId, options) {
      ...
      vm.menus[menuId].items.push({
        title: options.title || '',
        state: options.state || '',
        //stateParams: getStateParams(options.stateParams),
        stateParams: options.stateParams || {},
        type: options.type || 'item',
        class: options.class,
        icon: options.icon || 'glyphicon glyphicon-cog',
        isPublic:
          options.isPublic === null || typeof options.isPublic === 'undefined'
    ...
    function removeSubMenuItem(menuId, submenuItemURL) {
      ...
      // Search for menu item to remove
      angular.forEach(vm.menus[menuId].items, function (element, itemIndex) {
        angular.forEach(vm.menus[menuId].items[itemIndex].items, function (
          item,
          subitemIndex
        ) {
          if (
        ...
        if (~me.roles.indexOf('*')) {
          ...
          /*angular.forEach(user.roles, function (element, userRoleIndex) {
                        angular.forEach(me.roles, function(item, roleIndex) {
                            console.log("me.roles[roleIndex] = %s", me.roles[roleIndex]);
                            console.log("user.roles[userRoleIndex] = %s", user.roles[userRoleIndex]);
                            if (me.roles[roleIndex] === user.roles[userRoleIndex]) {
                                console.log("success matching rights !!!");
                                return true;
                                //res=true;
                                //break;
                            }
          ...
          // me.roles   = current logged user roles
          // user.roles = module roles
          for (var i = 0; i < me.roles.length; i++) {
            if (user.roles.indexOf(me.roles[i]) !== -1) {
              return true;
            }
          }
        }
